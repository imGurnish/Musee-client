...
class AdminCountriesPage extends StatefulWidget {
  const AdminCountriesPage({super.key});

  @override
  State<AdminCountriesPage> createState() => _AdminCountriesPageState();
}

class _AdminCountriesPageState extends State<AdminCountriesPage> {
  ...
  Future<void> _upsert({Map<String, dynamic>? existing}) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(existing == null ? 'Create country' : 'Edit country'),
        content: Column(
          ...
          children: [
            TextField(
              ...
              maxLength: 2,
              decoration: const InputDecoration(labelText: 'Code (2 letters)'),
            ),
            TextField(
              controller: nameCtrl,
    ...
    try {
      ...
      if (existing == null) {
        ...
        await client.patch(
          '${AppSecrets.backendUrl}/api/admin/countries/${existing['country_id']}',
          data: body,
          options: dio.Options(headers: _headers()),
        );
      }
  ...
  Future<void> _delete(String id) async {
    ...
    try {
      final client = GetIt.I<dio.Dio>();
      await client.delete(
        '${AppSecrets.backendUrl}/api/admin/countries/$id',
        options: dio.Options(headers: _headers()),
      );
      await _load();
    } on dio.DioException catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Admin • Countries'),
        actions: [
          IconButton(
            onPressed: () => _upsert(),
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          subtitle: Text(row['country_id']?.toString() ?? ''),
                          trailing: Wrap(
                            spacing: 4,
                            children: [
                              IconButton(
                              ...
                              IconButton(
                                onPressed: () =>
                                    _delete(row['country_id'].toString()),
